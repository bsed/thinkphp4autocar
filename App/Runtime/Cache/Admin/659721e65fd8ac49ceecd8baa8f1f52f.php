	...
	<ul class="doc-nav list-group sidebar-nav-v1" id="sidebar-nav" data-spy="pgy_affix" data-offset-top="202" data-offset-bottom="300">
		...
		<li class="list-group-item active">
			...
			<ul class="collapse in">
				<li id="zllb"><a href="__URL__/docs"> 资料列表</a></li>
				<li id="tjzl"><a href="__URL__/addDocs"> 添加资料</a></li>
			</ul>
		</li>
		<!-- End Manage App -->
	</ul>
                ...
                <table class="table table-striped table-hover text-center myapps my_history pgy-table">
                    <thead>
                        <tr class="pgy-warning">
                            ...
                            <th class="text-center" width="20%">删除</th>
                        </tr>
                    </thead>
                    <tbody>
                    	<?php if(is_array($docs)): foreach($docs as $key=>$doc): ?><tr class="my_history_row text-center">
	                            <td><?php echo ($doc["name"]); ?></td>
	                            <td>
	                                <a class="btn btn-danger btn-xs" onclick="javascript:void(0)" href="javascript:void(0);" id="<?php echo ($doc["id"]); ?>"><i class="fa fa-trash-o"></i> 删除</a>
	                            </td>
...
$('.btn-danger').each(function() {
	$(this).click(function() {
		...
		loading=1;
		var url = '__URL__/deleteDocs';
		$.post(url, params, function(d){
			if(d.error == ""){
				alert("删除成功");
	        	window.location.href="__URL__/docs";
			}else{
				alert(d.error);
			}
		}, 'json');
