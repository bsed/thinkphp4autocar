            ...
            <div class="panel pgy-panel-sea margin-bottom-40">
                <div class="panel-heading pgy-panel-heading">
                  <h3 class="panel-title" style="color:#fff">
                    <i class="fa fa-angle-double-right"></i>公共设施列表 &nbsp;
                    <!--  
                ...
                <table class="table table-striped table-hover text-center myapps my_history pgy-table">
                    <thead>
                        <tr class="pgy-warning">
                            <!-- <th class="text-center" width="20%">编号</th> -->
                            <th class="text-center" width="20%">名称</th>
                            <th class="text-center" width="20%">操作</th>
                        </tr>
                    </thead>
                    <tbody>
                    	<?php if(is_array($houses)): foreach($houses as $key=>$house): ?><tr class="my_history_row text-center">
	                            <!-- <td><?php echo ($house["number"]); ?></td> -->
	                            <td><?php echo ($house["name"]); ?></td>
	                            <td>
	                            	<a class="btn btn-info btn-xs" href="__URL__/editFacilities/id/<?php echo ($house["id"]); ?>"><i class="fa fa-edit"></i> 编辑</a>
	                                <a class="btn btn-danger btn-xs" onclick="javascript:void(0)" href="javascript:void(0);" id="<?php echo ($house["id"]); ?>"><i class="fa fa-trash-o"></i> 删除</a>
	                            </td>
...
$('.btn-danger').each(function() {
	$(this).click(function() {
		...
		loading=1;
		var url = '__URL__/deleteFacilities';
		$.post(url, params, function(d){
			if(d.error == ""){
				alert("删除成功");
	        	window.location.href="__URL__/facilities";
			}else{
				alert(d.error);
			}
		}, 'json');
