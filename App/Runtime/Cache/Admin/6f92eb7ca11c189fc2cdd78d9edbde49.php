            ...
            <div class="panel pgy-panel-sea margin-bottom-40">
                <div class="panel-heading pgy-panel-heading">
                  <h3 class="panel-title" style="color:#fff">
                    <i class="fa fa-angle-double-right"></i>编辑机柜 &nbsp;
                    <!--  
	            ...
	            <form class="reg-page sky-form" id="form" method="POST" action="__URL__/eidtRoomHandle" >
	            		<input type="hidden" name="id" value="<?php echo ($room["id"]); ?>" />
                        <section>
		                    <label class="label">名称</label>
		                    <label class="input">
		                        <i class="icon-append fa fa-pencil"></i>
		                        <input type="text" id="name" name="name" value="<?php echo ($room["name"]); ?>" placeholder="请输入名称">
		                    </label>
		                </section>
		                <div class="row">
...
function submitForm(){
    ...
    function complete(d){
        if (d.error == ""){
        	alert("保存成功");
        	window.location.href="__GROUP__/Machine/room";
        }else{
        	alert(d.error);
        	loading=0;
        }
