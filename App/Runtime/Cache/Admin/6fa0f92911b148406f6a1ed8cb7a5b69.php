            ...
            <div class="panel pgy-panel-sea margin-bottom-40">
                <div class="panel-heading pgy-panel-heading">
                  <h3 class="panel-title" style="color:#fff">
                    <i class="fa fa-angle-double-right"></i>商品列表 &nbsp;
                    <!--  
                ...
                <table class="table table-striped table-hover text-center myapps my_history pgy-table">
                    ...
                    <tbody>
                    	<?php if(is_array($goods)): foreach($goods as $key=>$goods): ?><tr class="my_history_row text-center">
	                            <td><?php echo ($goods["name"]); ?></td>
	                            <td>
	                                <a class="btn btn-info btn-xs" href="__URL__/editGoods/id/<?php echo ($goods["id"]); ?>"><i class="fa fa-edit"></i> 编辑</a>
	                                <a class="btn btn-danger btn-xs" onclick="javascript:void(0)" href="javascript:void(0);" id="<?php echo ($goods["id"]); ?>"><i class="fa fa-trash-o"></i> 删除</a>
	                            </td>
...
$('.btn-danger').each(function() {
	$(this).click(function() {
		...
		loading=1;
		var url = '__URL__/deleteGoods';
		$.post(url, params, function(d){
			if(d.error == ""){
				alert("删除成功");
	        	window.location.href="__URL__/goods";
			}else{
				alert(d.error);
			}
		}, 'json');
