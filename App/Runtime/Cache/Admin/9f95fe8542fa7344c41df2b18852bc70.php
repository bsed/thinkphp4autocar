        ...
        <div class="col-md-9">
           <div class="margin-bottom-20">
            <div class="panel pgy-panel-sea margin-bottom-40">
	            ...
	            <form class="reg-page sky-form" id="form" method="POST" action="__GROUP__/Category/addCategoryHandle" >
                        <section>
		                    <label class="label">名称</label>
		                    <label class="input">
		                        <i class="icon-append fa fa-pencil"></i>
		                        <input type="text" id="name" name="name" value="" placeholder="请输入分类名称">
		                    </label>
		                </section>
		                <section>
		                ...
		                <section id="superior-category">
		                    <label class="label">上级分类</label>
		                    <label class="select">
		                        <select name="superiorCategory" id="superiorCategory" >
		                        </select>
		                    </label>
		                </section>
		                <div class="row">
		                    ...
		                    <div class="col-md-6 col-xs-6 text-right">
		                        <button class="btn-u" type="submit" id="submitButton" onclick="submitForm()">添加分类</button>
		                    </div>
		                </div>
		        </form>
            </div>
...
$("#tjfl").attr("class","current");

$("#applevel").live('change',function(){
	var level = $("#applevel option:selected").val();
	if(level != "" && level != 1){
		$("#superior-category").css("display","block");
		var params = {};
		params.level = level - 1;
		getCategoryByLevel(params);
	}else{
		$("#superior-category").css("display","none");
	}
});

function getCategoryByLevel(params){
	var url = '__GROUP__/Category/getCategoryByLevel';
	$.post(url, params, function(d){
		if(d.error == ""){
			$("#superiorCategory").empty();
			$.each(d.categoryList,function(i,val){
				$("#superiorCategory").append("<option value='"+ val.id +"'>"+ val.name +"</option>");
			});
		}else{
			alert(d.error);
		}
...
function submitForm(){
    ...
    function checkForm(){
    	...
    	if(!$.trim ( $('#applevel').val() )){
    		alert('请填写应用名称');
    		return false;
    	}
    	if($.trim ( $('#applevel').val()) == 1 || $.trim ( $('#applevel').val()) == 2){
    		if(!$.trim ( $('#superiorCategory').val() )){
        		alert('请填写应用名称');
        		return false;
        	}
    	}
    ...
    function complete(d){
        if (d.error == ""){
        	alert("添加成功");
        	window.location.href="__GROUP__/Rbac/category"; 
        }else{
        	alert(d.error);
        	loading=0;
        }
