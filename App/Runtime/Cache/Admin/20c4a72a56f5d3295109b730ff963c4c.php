            ...
            <div class="panel pgy-panel-sea margin-bottom-40">
                <div class="panel-heading pgy-panel-heading">
                  <h3 class="panel-title" style="color:#fff">
                    <i class="fa fa-angle-double-right"></i>报修列表 &nbsp;
                    <!--  
                ...
                <table class="table table-striped table-hover text-center myapps my_history pgy-table">
                    <thead>
                        <tr class="pgy-warning">
                            <th class="text-center" width="20%">单元楼</th>
                            <th class="text-center" width="20%">业主姓名</th>
                            <th class="text-center" width="20%">报修内容</th>
                            <th class="text-center" width="20%">报修时间</th>
                            <th class="text-center" width="20%">操作</th>
                        </tr>
                    </thead>
                    <tbody>
                    	<?php if(is_array($repair)): foreach($repair as $key=>$repair): ?><tr class="my_history_row text-center">
	                            <td><?php echo ($repair["number"]); ?></td>
	                            <td><?php echo ($repair["nickname"]); ?></td>
	                            <td><?php echo ($repair["content"]); ?></td>
	                            <td><?php echo (time_tran($repair["created_time"])); ?></td>
	                            <td>
	                            	<a class="btn btn-info btn-xs" href="__URL__/repairComment/id/<?php echo ($repair["id"]); ?>"><i class="fa fa-edit"></i> 查看</a>
	                                <a class="btn btn-danger btn-xs" onclick="javascript:void(0)" href="javascript:void(0);" id="<?php echo ($repair["id"]); ?>"><i class="fa fa-trash-o"></i> 删除</a>
	                            </td>
...
$('.btn-danger').each(function() {
	$(this).click(function() {
		...
		loading=1;
		var url = '__URL__/deleteRepair';
		$.post(url, params, function(d){
			if(d.error == ""){
				alert("删除成功");
	        	window.location.href="__URL__/repair";
			}else{
				alert(d.error);
			}
		}, 'json');
