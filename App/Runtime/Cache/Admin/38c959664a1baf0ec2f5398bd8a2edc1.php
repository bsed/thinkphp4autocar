            ...
            <div class="panel pgy-panel-sea margin-bottom-40">
                <div class="panel-heading pgy-panel-heading">
                  <h3 class="panel-title" style="color:#fff">
                    <i class="fa fa-angle-double-right"></i>好友列表 &nbsp;
                    <!--  
                ...
                <table class="table table-striped table-hover text-center myapps my_history pgy-table">
                    <thead>
                        <tr class="pgy-warning">
                            ...
                            <th class="text-center" width="25%">性别</th>
                            <th class="text-center" width="25%">年龄</th>
                            <th class="text-center" width="25%">编辑</th>
                        </tr>
                    </thead>
                    <tbody>
                    	<?php if(is_array($friends)): foreach($friends as $key=>$friend): ?><tr class="my_history_row text-center">
	                            <td><?php echo ($friend["nickname"]); ?></td>
	                            <td><?php echo ($friend["gender"]); ?></td>
	                            <td><?php echo ($friend["age"]); ?></td>
	                            <td>
	                                <a class="btn btn-danger btn-xs" onclick="javascript:void(0)" href="javascript:void(0);" id="<?php echo ($user["id"]); ?>"  style="display:none;"><i class="fa fa-trash-o"></i> 删除</a>
	                            </td>
...
$('.btn-danger').each(function() {
	$(this).click(function() {
		...
		loading=1;
		var url = '__URL__/deleteMyFriend';
		$.post(url, params, function(d){
			if(d.error == ""){
				alert("删除成功");
	        	window.location.href="__URL__/myFriends";
			}else{
				alert(d.error);
			}
		}, 'json');
