            ...
            <div class="panel pgy-panel-sea margin-bottom-40">
                <div class="panel-heading pgy-panel-heading">
                  <h3 class="panel-title" style="color:#fff">
                    <i class="fa fa-angle-double-right"></i>添加资料 &nbsp;
                    <!--  
	            ...
	            <form class="reg-page sky-form" id="form" method="POST" action="__URL__/addDocsHandle" >
                        <section>
		                ...
		                <section>
		                    <label class="label">上传资料名</label>
		                    <label class="input">
		                        <input type="text" id="apkName" name="apkName" style="padding-left:400px;" value="" disabled="disabled" placeholder="上传资料获取">
		                    </label>
		                </section>
		                <section>
		                    <label class="input">
		                        <a href="javascript:void(0);" id="uploader-screenshot" class="btn-u btn-u-xs app_edit_screenshot_upload_button" style="position: relative; overflow: hidden; direction: ltr;"><i class="fa fa-picture-o"></i> 
		                        	上传资料<input id="docs" type="file" name="docs" style="position: absolute; right: 0px; top: 0px; font-family: Arial; font-size: 118px; margin: 0px; padding: 0px; cursor: pointer; opacity: 0;" /></a>
		                    </label>
		                </section>
		                <div class="row">
...
function ajaxFileUpload() {
    ...
    $.ajaxFileUpload
		(
			{
				url: '__GROUP__/Learn/uploadFileHandle',
				secureuri:false,
				fileElementId:'docs',
				dataType: 'json',
...
function submitForm(){
    ...
    function complete(d){
        if (d.error == ""){
        	alert("保存成功");
        	window.location.href="__GROUP__/Learn/docs";
        }else{
        	alert(d.error);
        	loading=0;
        }
