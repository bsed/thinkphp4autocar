            ...
            <div class="panel pgy-panel-sea margin-bottom-40">
                <div class="panel-heading pgy-panel-heading">
                  <h3 class="panel-title" style="color:#fff">
                    <i class="fa fa-angle-double-right"></i>资料列表 &nbsp;
                    <!--  
                ...
                <table class="table table-striped table-hover text-center myapps my_history pgy-table">
                    <thead>
                        <tr class="pgy-warning">
                            ...
                            <th class="text-center" width="20%">年龄</th>
                            <th class="text-center" width="20%">性别</th>
                            <th class="text-center" width="20%">房门号</th>
                            <th class="text-center" width="20%">操作</th>
                        </tr>
                    </thead>
                    <tbody>
                    	<?php if(is_array($residents)): foreach($residents as $key=>$resident): ?><tr class="my_history_row text-center">
	                            <td><?php echo ($resident["nickname"]); ?></td>
	                            <td><?php echo ($resident["age"]); ?></td>
	                            <td><?php echo ($resident["gender"]); ?></td>
	                            <td><?php echo ($resident["number"]); ?></td>
	                            <td>
	                            	<a class="btn btn-info btn-xs" href="__URL__/editResident/id/<?php echo ($resident["id"]); ?>"><i class="fa fa-edit"></i> 编辑</a>
	                                <a class="btn btn-danger btn-xs" onclick="javascript:void(0)" href="javascript:void(0);" id="<?php echo ($resident["id"]); ?>"><i class="fa fa-trash-o"></i> 删除</a>
	                            </td>
...
$('.btn-danger').each(function() {
	$(this).click(function() {
		...
		loading=1;
		var url = '__URL__/deleteUserInfo';
		$.post(url, params, function(d){
			if(d.error == ""){
				alert("删除成功");
	        	window.location.href="__URL__/resident";
			}else{
				alert(d.error);
			}
		}, 'json');
