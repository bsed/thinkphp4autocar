            ...
            <div class="panel pgy-panel-sea margin-bottom-40">
                <div class="panel-heading pgy-panel-heading">
                  <h3 class="panel-title" style="color:#fff">
                    <i class="fa fa-angle-double-right"></i>费用列表 &nbsp;
                    <!--  
                ...
                <table class="table table-striped table-hover text-center myapps my_history pgy-table">
                    <thead>
                        <tr class="pgy-warning">
                            <th class="text-center" width="20%">住户房号</th>
                            <th class="text-center" width="20%">月份</th>
                            <th class="text-center" width="10%">总费用</th>
                            <th class="text-center" width="20%">状态</th>
                            <th class="text-center" width="30%">操作</th>
                        </tr>
                    </thead>
                    <tbody>
                    	<?php if(is_array($cost)): foreach($cost as $key=>$cost): ?><tr class="my_history_row text-center">
	                            <td><?php echo ($cost["number"]); ?></td>
	                            <td><?php echo ($cost["date"]); ?></td>
	                            <td><?php echo ($cost["total"]); ?></td>
	                            <?php if($cost['status'] == 1): ?><td>已缴</td>
	                            <?php elseif($cost['status'] == 2): ?>
	                            	<td style="color:#32CD32">已确认</td>
	                            <?php else: ?>
	                            	<td>未缴</td><?php endif; ?>
	                            <td>
	                            	<?php if($cost['status'] == 1): ?><a class="btn btn-info btn-xs comfirm" href="javascript:void(0);" id="<?php echo ($cost["id"]); ?>"><i class="fa fa-edit"></i> 确认</a><?php endif; ?>
	                            	<a class="btn btn-info btn-xs" href="__URL__/costDetails/id/<?php echo ($cost["id"]); ?>"><i class="fa fa-edit"></i> 查看</a>
	                                <a class="btn btn-danger btn-xs" onclick="javascript:void(0)" href="javascript:void(0);" id="<?php echo ($cost["id"]); ?>"><i class="fa fa-trash-o"></i> 删除</a>
	                            </td>
...
<!-- JS Page Level -->
<script type="text/javascript">
$('.comfirm').each(function() {
	$(this).click(function() {
		params = {};
		params.id = $(this).attr("id");
		loading=1;
		var url = '__URL__/confirmCost';
		$.post(url, params, function(d){
			if(d.error == ""){
	        	window.location.href="__URL__/cost";
			}else{
				alert(d.error);
			}
		}, 'json');
