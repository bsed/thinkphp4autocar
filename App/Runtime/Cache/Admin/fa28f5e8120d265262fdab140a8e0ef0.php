	...
	<ul class="doc-nav list-group sidebar-nav-v1" id="sidebar-nav" data-spy="pgy_affix" data-offset-top="202" data-offset-bottom="300">
		...
		<li class="list-group-item active">
			...
			<ul class="collapse in">
				<li id="wzgl"><a href="__URL__/news"> 文章列表</a></li>
			</ul>
		</li>
		<!--  
		<li class="list-group-item active">
			<a href="javascript:void(0)">球场管理</a>
			<ul class="collapse in">
				<li id="qclb"><a href="__URL__/court"> 球场列表</a></li>
			</ul>
		</li>
		-->
		<!-- End Manage App -->
            ...
            <div class="panel pgy-panel-sea margin-bottom-40">
                <div class="panel-heading pgy-panel-heading">
                  <h3 class="panel-title" style="color:#fff">
                    <i class="fa fa-angle-double-right"></i>文章列表 &nbsp;
                    <!--  
                ...
                <table class="table table-striped table-hover text-center myapps my_history pgy-table">
                    ...
                    <tbody>
                    	<?php if(is_array($newsList)): foreach($newsList as $key=>$news): ?><tr class="my_history_row text-center">
	                            <td><?php echo ($news["title"]); ?></td>
	                            <td>
	                            	<?php if($news['status'] == 'F'): ?><span style="color:orange">未审核</span>
	                            	<?php elseif($news['status'] == 'U'): ?>
	                            		<span style="color:red">不通过</span>
	                            	<?php else: ?>
	                            		<span style="color:green">通过</span><?php endif; ?>
	                            </td>
	                            <td><?php echo ($news["created_by"]); ?></td>
	                            <td><?php echo (date("Y-m-d",$news["created_time"])); ?></td>
	                            <td>
	                                <a class="btn btn-info btn-xs" href="__URL__/editNews/id/<?php echo ($news["id"]); ?>"><i class="fa fa-edit"></i> 编辑</a>
	                                <a class="btn btn-danger btn-xs" onclick="javascript:void(0)" href="javascript:void(0);" id="<?php echo ($news["id"]); ?>"><i class="fa fa-trash-o"></i> 删除</a>
	                            </td>
...
$('.btn-danger').each(function() {
	$(this).click(function() {
		...
		$.post(url, params, function(d){
			if(d.error == ""){
				alert("删除成功");
	        	window.location.href="__URL__/news";
			}else{
				alert(d.error);
			}
		}, 'json');
