                ...
                <table class="table table-striped table-hover text-center myapps my_history pgy-table">
                    <thead>
                        <tr class="pgy-warning">
                            ...
                            <th class="text-center" width="16%">真实姓名</th>
                            <th class="text-center" width="16%">等级</th>
                            <th class="text-center" width="17%">最近登录</th>
                            <th class="text-center" width="17%">使用时长</th>
                            <th class="text-center" width="18%">编辑</th>
                        </tr>
                    </thead>
                    <tbody>
                    	<?php if(is_array($wuser)): foreach($wuser as $key=>$wuser): ?><tr class="my_history_row text-center">
	                            ...
	                            <td>
	                                <a class="btn btn-info btn-xs" onclick="javascript:void(0)" href="javascript:void(0);" uid="<?php echo ($wuser["id"]); ?>" ><i class="fa fa-edit"></i> 恢复</a>
	                            </td>
...
$('.btn-info').each(function() {
	$(this).click(function() {
		...
		loading=1;
		var url = '__URL__/CancelPullBlackHandle';
		$.post(url, params, function(d){
			if(d.error == ""){
				alert("已恢复");
	        	window.location.href="__URL__/blackList";
			}else{
				alert(d.error);
			}
		}, 'json');
