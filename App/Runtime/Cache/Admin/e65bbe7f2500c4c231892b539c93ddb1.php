            ...
            <div class="panel pgy-panel-sea margin-bottom-40">
                <div class="panel-heading pgy-panel-heading">
                  <h3 class="panel-title" style="color:#fff">
                    <i class="fa fa-angle-double-right"></i>添加兼职 &nbsp;
                   </h3>
                </div>
	            <form class="reg-page sky-form" id="form" method="POST" action="__GROUP__/Wuzhouyuan/addJobsHandle" >
                    <section>
	                    <label class="label">标题</label>
	                    <label class="input">
	                        <i class="icon-append fa fa-pencil"></i>
	                        <input type="text" id="title" name="title" value="" placeholder="请输入兼职名称">
	                    </label>
	                </section>
	                <section>
	                    <label class="label">内容</label>
	                    <label class="textarea textarea-resizable">
	                        <textarea rows="6" placeholder="请输入兼职内容" id="content" name="content"></textarea>
	                    </label>
	                </section>
	                <div class="row">
	                    ...
	                    <div class="col-md-6 col-xs-6 text-right">
	                        <button class="btn-u" type="submit" id="submitButton" onclick="submitForm()">添加兼职</button>
	                    </div>
	                </div>
		        </form>
            </div>
...
function submitForm(){
    ...
    function complete(d){
        if (d.error == ""){
        	alert("添加成功");
        	window.location.href="__GROUP__/Wuzhouyuan/jobs"; 
        }else{
        	alert(d.error);
        	loading=0;
        }
