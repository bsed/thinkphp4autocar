            ...
            <div class="panel pgy-panel-sea margin-bottom-40">
                <div class="panel-heading pgy-panel-heading">
                  <h3 class="panel-title" style="color:#fff">
                    <i class="fa fa-angle-double-right"></i>出租停车位 &nbsp;
                    <!--  
	            ...
	            <form class="reg-page sky-form" id="form" method="POST" action="__URL__/editParkingHandle" >
	            		<input type="hidden" name="id" value="<?php echo ($parking["id"]); ?>" />
                        <section>
		                    <label class="label">编号</label>
		                    <label class="input">
		                        <i class="icon-append fa fa-pencil"></i>
		                        <input type="text" id="number" name="number" value="<?php echo ($parking["number"]); ?>" disabled="disabled" style="background:#eeeeee">
		                    </label>
		                </section>
		                <section>
		                    ...
		                    <label class="input">
		                        <i class="icon-append fa fa-pencil"></i>
		                        <input type="text" id="name" name="name" value="<?php echo ($parking["name"]); ?>" disabled="disabled" style="background:#eeeeee">
		                    </label>
		                </section>
		                <section>
		                    <label class="label">选择租户</label>
		                    <label class="select">
		                        <select name="residentId" id="residentId" >
		                        	<option value="">请选择</option>
									<?php if(is_array($resident)): foreach($resident as $key=>$resident): ?><option value="<?php echo ($resident["id"]); ?>"><?php echo ($resident["nickname"]); ?></option><?php endforeach; endif; ?>
		                        </select>
		                    </label>
		                </section>
		                <div class="row">
		                    ...
		                    <div class="col-md-6 col-xs-6 text-right">
		                        <button class="btn-u" type="submit" id="submitButton" onclick="submitForm()">出租</button>
		                    </div>
		                </div>
		        </form>
            </div>
...
function submitForm(){
    ...
    function complete(d){
        if (d.error == ""){
        	alert("出租成功");
        	window.location.href="__GROUP__/Property/parking";
        }else{
        	alert(d.error);
        	loading=0;
        }
